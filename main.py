    ...
    for row in sql_rows:
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.warning(f"Attempt to post {payload} returned status code: {resp.status_code}")
...
def post_image_to_species(poster, image_data_filepath):
    try:
        image_location_reader = BRAHMSExportReader(file_path=image_data_filepath)
        img_rows = image_location_reader.get_rows()
        next(img_rows)  # Skip header row
    except UnicodeDecodeError:
        image_location_reader = BRAHMSExportReader(file_path=image_data_filepath, encoding='utf-16le')
        img_rows = image_location_reader.get_rows()
        next(img_rows)  # Skip header row
    ...
    for row in img_rows:
        ...
        if content['count'] == 1:
            ...
            if resp.status_code != 200:
                logger.warning(f"Attempt to post {img_filepath} for species {species_pk} returned status code: "
                               f"{resp.status_code}")
...
def main():
    ...
    if username is None or password is None:
        logger.error("Username and password must be set as environment variables.")
